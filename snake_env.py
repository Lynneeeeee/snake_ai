...
class Snake:
    ...
    def handle_input(self, action):
        # AI doesn't need key press: keys = pygame.key.get_pressed()
        # actions is one of [straight, right, left]
...
class Game:
    ...
    def __init__(self, Width=640, Height=480):
        ...
        self.nS = 20  # input of nural network. get new 20 messages
        self.nA = 3  # output... 3 types of stratergy
        self.reset()
        # self.position_berry()

    def reset(self):
        # a new method, init game state
        self.score = 0
        self.frame = 0
        self.snake = Snake(self.block_size)
        self.position_berry()
        self.total_step = 0
        self.reward = 0  # reward to AI, to guide AI play better
    ...
    def berry_collision(self):
        ...
        if (head.x == self.berry.position.x and
            ...
            self.position_berry()
            self.score += 1
            self.reward = 10  # give reward
        else:
            self.snake.blocks.pop()

    def head_hit_body(self, position=None):
        # changed, added a new para position
        # detect blocks around head, like a radar
        if position is None:
    ...
    def play_step(self, action):
        ...
        self.snake.handle_input(action)
    ...
    def get_state(self):
        # new method, 20 messages(states) need to know
        head = self.snake.blocks[0]
        # 4 blocks radar around head
        ...
        # detect if the head will hit body/wall
        ...
        # distant radar, like a straight laser from head
        # detects if there is part of body in the forward path
